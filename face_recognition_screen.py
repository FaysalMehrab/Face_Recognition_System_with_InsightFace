...
def run_screen_recognition(embeddings_folder="Embeddings", threshold=0.4, monitor=1):
    """Run real-time face recognition on your computer screen (multi-face)."""
    with mss.mss() as sct:
        monitor_info = sct.monitors[monitor]  # monitor=1 is usually the primary screen
        frame_count = 0
        while True:
            # Capture screen
            ...
            # Resize to medium size for display (e.g., width=900, keep aspect ratio)
            display_width = 900
            h, w = frame.shape[:2]
            ...
            if frame_count % 2 == 0:  # Process every 2nd frame
                results = recognize_faces_from_frame(frame, embeddings_folder, threshold)
                # Adjust bbox coordinates to resized frame
                for name, similarity, bbox in results:
                    x1, y1, x2, y2 = [int(coord * scale) for coord in bbox]
                    color = (0, 255, 0) if name != "Unknown" else (0, 0, 255)  # Green or Red
                    ...
                    cv2.putText(
                        display_frame, text, (x1, y1 - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2
                    )

                cv2.imshow("Screen Face Recognition", display_frame)
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break
...
if __name__ == "__main__":
    ...
    # To use screen for real-time recognition, uncomment:
    run_screen_recognition("Embeddings")